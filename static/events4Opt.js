...
angular.module('awesome.services.events4Opt', [])
	.provider('events4Opt', function() {
				...
				if (eventRef) {
					...
					for (var i10 = 0, length10 = eventRef.singleListeners.length; i10 < length10; i10++) {if (eventRef.singleListeners[i10] !== null) {
						...
						if (config.passThroughArguments)
							...
							// Apply the listener
							eventRef.singleListeners[i10].listener.apply(null, eventArguments);

						// If they don't need to be passed through
						else

							// Call the listener
							eventRef.singleListeners[i10].listener(eventName);
