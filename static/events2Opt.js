...
angular.module('awesome.services.events2Opt', [])
	.provider('events2Opt', function() {
		...
		this.$get = function eventsFactory(
				...
				// Else assume this is a array
					...
					if (checkMemory) {
						...
						for (var i11 = 0, length11 = newListenerObject.eventNamesFlat.length; i11 < length11; i11++) {if (newListenerObject.eventNamesFlat[i11] !== null) {
							...
							// If an eventMemoryObject is found
							if (findByKey(eventMemoryObjects, 'newListenerObject.eventNamesFlat[i11]', newListenerObject.eventNamesFlat[i11]))
			...
			self.dispatchEvent = function(eventName) {
				...
				if (eventMemoryObject)
					...
					eventMemoryObject.arguments = arguments;
				...
				if (eventRef) {
					...
					for (var i10 = 0, length10 = eventRef.singleListeners.length; i10 < length10; i10++) {if (eventRef.singleListeners[i10] !== null) {
						...
						// Apply the listener
						eventRef.singleListeners[i10].listener.apply(null, arguments);
